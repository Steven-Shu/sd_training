! function(e, t) {
	...
	try {
					...
					switch (c && "object" == typeof c && "function" == typeof c.toJSON && (c = c.toJSON(e)), "function" == typeof rep && (c = rep.call(t, e, c)), typeof c) {
						...
						case "object":
							...
							if (rep && "object" == typeof rep)
								...
								for (n in c) Object.prototype.hasOwnProperty.call(c, n) && (s = str(n, c), s && i.push(quote(n) + (gap ? ": " : ":") + s));
							return s = 0 === i.length ? "{}" : gap ? "{\n" + gap + i.join(",\n" + gap) + "\n" + o + "}" : "{" + i.join(",") + "}", gap = o, s
					}
				}
				var rx_one = /^[\],:{}\s]*$/,
		...
		sd.para_default = {
			...
			if ("object" == typeof sd.para.source_type && (sd.para.source_type.utm = _.isArray(sd.para.source_type.utm) ? sd.para.source_type.utm.concat(r) : r, sd.para.source_type.search = _.isArray(sd.para.source_type.search) ? sd.para.source_type.search.concat(n) : n, sd.para.source_type.social = _.isArray(sd.para.source_type.social) ? sd.para.source_type.social.concat(s) : s, sd.para.source_type.keyword = _.isObject(sd.para.source_type.keyword) ? _.extend(a, sd.para.source_type.keyword) : a), _.isObject(sd.para.heatmap) && (sd.para.heatmap.clickmap = sd.para.heatmap.clickmap || "default", sd.para.heatmap.scroll_notice_map = sd.para.heatmap.scroll_notice_map || "default", sd.para.heatmap.scroll_delay_time = sd.para.heatmap.scroll_delay_time || 4e3, sd.para.heatmap.renderRefreshTime = sd.para.heatmap.renderRefreshTime || 1e3, sd.para.heatmap.loadTimeout = sd.para.heatmap.loadTimeout || 1e3), "object" == typeof sd.para.server_url && sd.para.server_url.length)
		...
		logger.info = function() {
			...
			function() {
					...
					n = _.each = function(t, n, s) {
						...
						if (e && t.forEach === e) t.forEach(n, s);
							...
							for (var a = 0, i = t.length; a < i; a++)
								if (a in t && n.call(s, t[a], a, t) === r) return !1
						} else
							for (var o in t)
				...
				var t, r, n, s = "",
					a = 0;
				for (t = r = 0, a = e.length, n = 0; n < a; n++) {
				...
				var t, r, n, s, a, i, o, c, u = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",
					l = 0,
					d = 0,
					p = "",
					f = [];
				if (!e) return e;
				e = _.utf8Encode(e);
				do t = e.charCodeAt(l++), r = e.charCodeAt(l++), n = e.charCodeAt(l++), c = t << 16 | r << 8 | n, s = c >> 18 & 63, a = c >> 12 & 63, i = c >> 6 & 63, o = 63 & c, f[d++] = u.charAt(s) + u.charAt(a) + u.charAt(i) + u.charAt(o); while (l < e.length);
				...
				var e = function() {
					...
					r = function(e) {
						function t(e, t) {
							var r, n = 0;
							for (r = 0; r < t.length; r++) n |= a[r] << 8 * r;
							return e ^ n
						}
						var r, n, s = navigator.userAgent,
							a = [],
							i = 0;
						for (r = 0; r < s.length; r++) n = s.charCodeAt(r), a.unshift(255 & n), a.length >= 4 && (i = t(i, a), a = []);
				...
				function t(t, r, n) {
					var s = function(s) {
						if (s = s || e(window.event)) {
							...
							var a, i, o = !0;
							return "function" == typeof n && (a = n(s)), i = r.call(t, s), !1 !== a && !1 !== i || (o = !1), o
						}
					};
					return s
				}
				...
				var s = "pushState" in window.history ? "popstate" : "hashchange";
				...
				set: function(e, t, r, n, s) {
					...
					s && (o = "; secure"), document.cookie = e + "=" + encodeURIComponent(t) + i + "; path=/" + a + o
				},
				remove: function(e, t) {
				...
				function n(e, r) {
					for (var n = e.charAt(0), s = r.split("&"), a = [], i = {}, o = [], c = e.substring(1), u = 0, l = s.length; u < l; u++)
						if (a = s[u].match(/(.*?)=(.*)/), a || (a = [s[u], s[u], ""]), "" !== a[1].replace(/\s/g, "")) {
				...
				return function(t, s) {
					...
					if (t = t.toString(), a = s.match(/^mailto:([^\/].+)/)) i.protocol = "mailto", i.email = a[1];
					else {
						if ((a = s.match(/(.*?)\/#\!(.*)/)) && (s = a[1] + a[2]), (a = s.match(/(.*?)#(.*)/)) && (i.hash = a[2], s = a[1]), i.hash && t.match(/^#/)) return n(t, i.hash);
						if ((a = s.match(/(.*?)\?(.*)/)) && (i.query = a[2], s = a[1]), i.query && t.match(/^\?/)) return n(t, i.query);
						if ((a = s.match(/(.*?)\:?\/\/(.*)/)) && (i.protocol = a[1].toLowerCase(), s = a[2]), (a = s.match(/(.*?)(\/.*)/)) && (i.path = a[2], s = a[1]), i.path = (i.path || "").replace(/^([^\/])/, "/$1").replace(/\/$/, ""), t.match(/^[\-0-9]+$/) && (t = t.replace(/^([^\/])/, "/$1")), t.match(/^\//)) return r(t, i.path.substring(1));
						if (a = r("/-1", i.path.substring(1)), a && (a = a.match(/(.*?)\.(.*)/)) && (i.file = a[0], i.filename = a[1], i.fileext = a[2]), (a = s.match(/(.*)\:([0-9]+)$/)) && (i.port = a[2], s = a[1]), (a = s.match(/(.*?)@(.*)/)) && (i.auth = a[1], s = a[2]), i.auth && (a = i.auth.match(/(.*)\:(.*)/), i.user = a ? a[1] : i.auth, i.pass = a ? a[2] : void 0), i.hostname = s.toLowerCase(), "." === t.charAt(0)) return r(t, i.hostname);
						e() && (a = i.hostname.match(e()), a && (i.tld = a[3], i.domain = a[2] ? a[2] + "." + a[3] : void 0, i.sub = a[1] || void 0)), i.port = i.port || ("https" === i.protocol ? "443" : "80"), i.protocol = i.protocol || ("443" === i.port ? "https" : "http")
					}
					return t in i ? i[t] : "{}" === t ? i : ""
				}
				...
				if (a) {
					...
					return i && i[0] ? "\u4ed8\u8d39\u5e7f\u544a\u6d41\u91cf" : e(r, s) ? "\u81ea\u7136\u641c\u7d22\u6d41\u91cf" : e(n, s) ? "\u793e\u4ea4\u7f51\u7ad9\u6d41\u91cf" : "" === s ? "\u76f4\u63a5\u6d41\u91cf" : "\u5f15\u8350\u6d41\u91cf"
				}
				return "\u83b7\u53d6url\u5f02\u5e38"
			}, _.info = {
				initPage: function() {
					...
					e && !r && _.jssdkDebug("referrer_domain\u5f02\u5e38_" + e + "_" + r), a || _.jssdkDebug("url_domain\u5f02\u5e38_" + n + "_" + a), this.pageProp = {
						referrer: e,
						referrer_host: t,
						referrer_domain: r,
						url: n,
				...
				campaignParamsStandard: function(e, t) {
					...
					var r = _.info.campaignParams(),
						n = {},
						s = {};
					for (var a in r)(" " + source_channel_standard + " ").indexOf(" " + a + " ") !== -1 ? n[e + a] = r[a] : s[t + a] = r[a];
					return {
						$utms: n,
						otherUtms: s
					}
			...
			if (e = JSON.stringify(e), logger.info(n), this.requestData = {
				...
				if (/sensors-verify/.test(navigator.userAgent)) {
					...
					if (a && a[0] && "string" == typeof a[1] && 2 === a[1].split("?").length) {
						...
						i && i === a[0] && o && o === a[1] ? (s = document.createElement("iframe"), s.setAttribute("src", "sensorsanalytics://trackEvent?event=" + encodeURIComponent(JSON.stringify(_.extend({
							server_url: sd.para.server_url
		...
		var saNewUser = {
				...
				checkIsFirstLatest: function() {
					...
					store.setProps(n, !0);
					var a = {};
					if ("" === e && (e = "url\u89e3\u6790\u5931\u8d25"), _.each(sd.para.is_track_latest, function(r, n) {
							if (r) {
								if ("utm" !== n && "url\u89e3\u6790\u5931\u8d25" === e) a["$latest_" + n] = "url\u7684domain\u89e3\u6790\u5931\u8d25";
			...
			commonWays = {
				...
				trackHeatMap: function(e, t) {
					if ("object" == typeof e && e.tagName) {
						...
						"button" !== r && "a" !== r && "a" !== n && "button" !== n && "input" !== r && "textarea" !== r && heatmap.start(null, e, r, t)
					}
				},
				trackAllHeatMap: function(e, t) {
					if ("object" == typeof e && e.tagName) {
						var r = e.tagName.toLowerCase();
						heatmap.start(null, e, r, t)
					}
				},
				autoTrackSinglePage: function(e, t) {
					...
					function n(e, t) {
						sd.track("$pageview", _.extend({
							$referrer: s,
							$referrer_host: s ? _.url("hostname", s) : s,
							$url: location.href,
							$url_path: location.pathname,
							$title: document.title
						}, e, r()), t), s = location.href
					}
					if (this.autoTrackIsUsed) var s = _.info.pageProp.url;
		...
		var heatmap = {
			setNotice: function(e) {
				sd.is_heatmap_render_mode = !0, sd.para.heatmap || (sd.errorMsg = "\u60a8SDK\u6ca1\u6709\u914d\u7f6e\u5f00\u542f\u70b9\u51fb\u56fe\uff0c\u53ef\u80fd\u6ca1\u6709\u6570\u636e\uff01"), e && e[0] && e[1] && "http:" === e[1].slice(0, 5) && "https" === location.protocol && (sd.errorMsg = "\u60a8\u7684\u5f53\u524d\u9875\u9762\u662fhttps\u7684\u5730\u5740\uff0c\u795e\u7b56\u5206\u6790\u73af\u5883\u4e5f\u5fc5\u987b\u662fhttps\uff01"), sd.para.heatmap_url || (sd.para.heatmap_url = location.protocol + "//static.sensorsdata.cn/sdk/" + LIB_VERSION + "/heatmap.min.js")
			},
			...
			selector: function(e) {
				...
				return e.getAttribute && e.getAttribute("id") && "not_use_id" !== sd.para.heatmap.element_selector ? "#" + e.getAttribute("id") : e.tagName.toLowerCase() + (~t ? ":nth-of-type(" + (t + 1) + ")" : "")
			},
			...
			initScrollmap: function() {
				...
				var e = function(e) {
					...
					t = e({
						timeout: 1e3,
						func: function(e) {
							var t = document.documentElement.scrollTop || window.pageYOffset || document.body.scrollTop || 0,
								r = new Date,
			...
			initHeatmap: function() {
				...
				return !(!_.isObject(sd.para.heatmap) || "default" !== sd.para.heatmap.clickmap) && (!(_.isFunction(sd.para.heatmap.collect_url) && !sd.para.heatmap.collect_url()) && ("all" === sd.para.heatmap.collect_elements ? sd.para.heatmap.collect_elements = "all" : sd.para.heatmap.collect_elements = "interact", void("all" === sd.para.heatmap.collect_elements ? _.addEvent(document, "click", function(t) {
					...
					"a" === a || "button" === a ? e.start(r, n.parentNode, n.parentNode.tagName.toLowerCase()) : e.start(r, n, s)
				}) : _.addEvent(document, "click", function(t) {
					var r = t || window.event;
					if (!r) return !1;
					var n = r.target || r.srcElement;
