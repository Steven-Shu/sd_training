...
func GetSAInstance(configs map[string]string) *MySAAnalytics {
	initOnceOnly.Do(func() {
		msaaInstance = new(MySAAnalytics)
		msaaInstance.configs = configs
...
func (msda *MySAAnalytics) initSDK() {
	...
	case Logging:
		consumer, err = sdk.InitLoggingConsumer(msaaInstance.configs["logagent_path"], hourcut)
	case Default:
		consumer, err = sdk.InitConcurrentLoggingConsumer(msaaInstance.configs["logagent_path"], hourcut)
	case Debug:
		consumer, err = sdk.InitDebugConsumer(msaaInstance.configs["remote_sensors"], debugWrite, TimeOut)
	default:
	...
	msda.sda = sdk.InitSensorsAnalytics(consumer, msaaInstance.configs["sensors_project"], false)
	msda.RegisterSuperProperties(map[string]interface{}{"appName": "神澈教育"})
	log.Debugf("sa initilization finished with url:%s finished", msaaInstance.configs["remote_sensors"])
}
